...
-- Update the Fields in the duplicates
	UPDATE CLNMAS SET DOB = (???) WHERE (CLIENT_ID = New_CLIENT_ID) and DOB is Null;

	  select *
	  from clnmas
	  WHERE (CLIENT_ID = New_CLIENT_ID) and DOB is Null
	  inner join
	  (select new_client_id, dob
	  from clnmas
	  where dob is not null) T
	  on clnmas.new_client_id = clnmas.client_id

-- Remove the Duplicates from CLNMAS;
    DELETE FROM CLNMAS WHERE (CLIENT_ID <> New_CLIENT_ID);
